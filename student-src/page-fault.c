...
pfn_t pagefault_handler(vpn_t request_vpn, int write) {
   ...
   */
    if(current->pagetable[victim_pfn].used)
    { 
      if(current->pagetable[victim_pfn].dirty)
      {
        page_to_disk(victim_pfn, victim_vpn, victim_pcb->pid);
      }
      current->pagetable[victim_pfn].valid = 0;
 ...
 // rlt[victim_pfn].pcb = current; 
  ...
  /* FIX ME */
  printf("Updatinge the pagetable\n");
  current->pagetable = current_pagetable;
  current->pagetable->valid = 0; 
